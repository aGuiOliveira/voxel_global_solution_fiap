...
def top3d(
    ...
    # Non-design region: voxels that must always be solid (xPhys = 1).
    # Typically wraps support and force-application points so that the
    # optimizer cannot remove material at the boundary conditions.
    if solid_mask is None:
        solid_mask = np.zeros((nely, nelx, nelz), dtype=bool)
    # A voxel cannot be both obstacle (void) and solid; obstacle wins to avoid
    # contradictory constraints. The caller is responsible for sane masks.
    solid_mask = solid_mask & ~obstacle_mask
    design_nele = nele - obstacle_mask.sum() - solid_mask.sum()
    logger.debug(
        f"design elements: {design_nele}/{nele} "
        f"(obstacles: {int(obstacle_mask.sum())}, solid: {int(solid_mask.sum())})"
    )
    ...
    # Combined "fixed" mask: cells whose density is not a design variable.
    # Passed to OC so they're excluded from the volume-bisection design domain.
    fixed_mask = obstacle_mask | solid_mask
    if gpu:
        solid_gpu = cp.asarray(solid_mask)
        fixed_gpu = obstacle_gpu | solid_gpu
    ...
    while change > tolx and loop < maxloop:
        ...
        if gpu:
            ...
            dc_gpu[fixed_gpu] = dv_gpu[fixed_gpu] = 0.0
            dv_gpu += 1e-9
            ...
            xnew_gpu, change = optimality_criteria_update(
                ...
                design_nele,
                use_gpu=True,
                solid_mask=solid_gpu,
            )
            xnew_gpu[obstacle_gpu] = 0.0
            xnew_gpu[solid_gpu] = 1.0
            xPhys_gpu = apply_filter(H_gpu, xnew_gpu, Hs_gpu, xnew_gpu.shape, use_gpu=True)
            xPhys_gpu[obstacle_gpu] = 0.0
            xPhys_gpu[solid_gpu] = 1.0
            x_gpu = xnew_gpu
            current_vol = cp.mean(xPhys_gpu[~fixed_gpu]).item() if (~fixed_gpu).any() else 0.0
        ...
        # ================================================= CPU
            ...
            dc[fixed_mask] = dv[fixed_mask] = 0.0
            dv += 1e-9
            ...
            xPhys[solid_mask] = 1.0
            x = xnew
            current_vol = xPhys[~fixed_mask].mean() if (~fixed_mask).any() else 0.0
        ...
        if iter_callback is not None:
            try:
                iter_callback(loop, cp.asnumpy(xPhys_gpu) if gpu else xPhys)
            except Exception as cb_err:
                logger.warning(f"iter_callback raised {type(cb_err).__name__}: {cb_err}")
