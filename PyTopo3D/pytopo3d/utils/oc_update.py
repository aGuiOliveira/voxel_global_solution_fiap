...
def optimality_criteria_update(
    ...
    design_nele: int,
    use_gpu: bool = False,
    solid_mask: Optional[Union[np.ndarray, "cp.ndarray"]] = None,
    ...
    # Note: parens grouping the whole RHS are required so that the
    # `isinstance(H, cusp.csr_matrix)` lookup is short-circuited when
    # HAS_CUPY is False (cusp is unbound on CPU-only installs).
    ...
    if use_gpu_for_calc:
        ...
        if solid_mask is None:
            solid_mask_gpu = cp.zeros_like(obstacle_mask_gpu)
        elif isinstance(solid_mask, cp.ndarray):
            solid_mask_gpu = solid_mask
        else:
            solid_mask_gpu = cp.asarray(solid_mask)

        # design_cells excludes BOTH obstacle (forced 0) and solid (forced 1)
        # — only the free design domain counts towards the volume constraint.
        design_cells_gpu = ~(obstacle_mask_gpu | solid_mask_gpu)
        ...
        while (l2 - l1) / (l1 + l2) > 1e-3:
            ...
            x_candidate_gpu[solid_mask_gpu] = 1.0
            ...
            xPhysCandidate_gpu[solid_mask_gpu] = 1.0
        ...
        xnew = x.copy()
        solid_mask_cpu = (
            np.zeros_like(obstacle_mask) if solid_mask is None else solid_mask
        )
        # Exclude both obstacle (forced 0) and solid (forced 1) from the
        # bisection's volume domain.
        design_cells = ~(obstacle_mask | solid_mask_cpu)
