...
def voxel_to_stl(
    ...
    # 6. Apply Taubin smoothing to the mesh if requested.
    # Patch local: trocado de Laplacian para Taubin. Laplacian puro com lamb=0.5
    # distorce vertices da fronteira (especialmente faces planas de keep_solid
    # nas bordas do design space — face do apoio do cantilever, p.ex.), gerando
    # triangulos longos e finos no STL final. Taubin alterna passos lambda+
    # (shrink) e lambda- (expand) e e' volume-preserving, sem distorcer bordas.
    #
    # Patch local (spikes): em paredes finas / pontos de pinca da topologia,
    # o marching cubes emite faces degeneradas (area~0) e arestas non-manifold
    # (compartilhadas por >2 faces). Esses vertices envenenam o operador
    # laplaciano do Taubin e ele dispara alguns vertices pra longe, gerando
    # "spikes" que apontam pra origem e quebram o formato. Limpar essas faces
    # ANTES do smoothing elimina os spikes (e de quebra torna a malha
    # watertight). Sem essa limpeza, ratio aresta_max/mediana ~20x; com ela ~1.6x.
    if smooth_mesh and smooth_iterations > 0:
        if len(mesh.vertices) > 0:
            ...
            trimesh.smoothing.filter_taubin(
                mesh, lamb=0.5, nu=0.53, iterations=smooth_iterations
            )
